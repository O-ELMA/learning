...
fn validate(x: &i16, y: &i16, z: &i16) -> Result<Color, IntoColorError> {
    let valid_range = 0..256;

    if !valid_range.contains(x) || !valid_range.contains(y) || !valid_range.contains(z) {
...
impl TryFrom<(i16, i16, i16)> for Color {
    ...
    fn try_from(tuple: (i16, i16, i16)) -> Result<Self, Self::Error> {
        validate(&(tuple.0), &(tuple.1), &(tuple.2))
    }
}
...
impl TryFrom<&[i16]> for Color {
    ...
    fn try_from(slice: &[i16]) -> Result<Self, Self::Error> {
        ...
        validate(&slice[0], &slice[1], &slice[2])
    }
}
